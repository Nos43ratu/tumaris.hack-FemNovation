...
func (u *UserRepo) GetByEmail(email string) (*models.UserInfo, error) {
	...
	err := u.db.QueryRow(ctx, query, email).Scan(&user.ID, &user.Email, &user.Phone, &user.FirstName, &user.LastName, &user.Password, &user.Role, &user.AboutMe, &user.Instagram, &user.Rating, &user.ShopID)
	if err != nil {
